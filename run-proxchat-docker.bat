...
docker run --rm --network=proxchat_default -v "%CD%\data:/data" -e DB_USERNAME=root -e DB_PASSWORD=mysql -e DB_HOST=db -e DB_NAME=process_data python:3.9 /bin/bash -c "pip install -r /data/requirements.txt && python /data/csv_to_db_docker.py"
